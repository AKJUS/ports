...
_MODPY_RUNBIN =		cd ${MODPY_BUILD_DIR} && ${SETENV} ${MAKE_ENV} ${MODPY_BIN}
MODPY_CMD =		${_MODPY_RUNBIN} ./${MODPY_SETUP} ${MODPY_SETUP_ARGS}
...
.  else
MODPY_TEST_LIBDIR ?=	${WRKDIR}/pytest-instroot${MODPY_SITEPKG}:${WRKINST}${MODPY_SITEPKG}:${WRKSRC}/build/lib:${WRKSRC}/build/lib.openbsd-${OSREV}-${ARCH}-cpython-${MODPY_MAJORMINOR}:${WRKSRC}/lib.openbsd-${OSREV}-${ARCH}-${MODPY_VERSION}:${WRKSRC}
.  endif
...
MODPY_TEST_TARGET +=	\
	rm -rf "${WRKDIR}/pytest-instroot"; \
	${INSTALL_DATA_DIR} ${WRKINST}${MODPY_LIBDIR}; \
	${_MODPY_RUNBIN} -m installer -d ${WRKDIR}/pytest-instroot ${MODPY_BUILD_DIR}/${_MODPY_WHEELSDIR}/*.whl;
